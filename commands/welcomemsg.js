	...
	action: args => {
		...
		Server.send(welcomeChannel, `__**Welcome to the MLP-VectorClub's Discord Server!**__`).then(() => {
			Server.send(welcomeChannel, `We have a few rules that you should keep in mind:\n\n${Server.getRules()}`).then(() => {
				Server.send(welcomeChannel, `Please send the command **/read** to this channel to reveal the rest of the channels on our server and start chatting. You can always get this information again by running the \`/rules\` command.`).catch(err => {
					failure(err, args);
				});
			}).catch(function(err){
				failure(err, args);
